...
class Spazi{
	...
	function Spazi(){
		add_action( 'init', array('Spazi','crea_custom_spazi'));
		add_filter( 'post_updated_messages', array('Spazi','update_spazi_messages'));
		add_action( 'save_post', array('Spazi','save_spazi_dettagli'));
		add_action('add_meta_boxes',array('Spazi','new_spazi_box'));
	}
	function new_spazi_box(){
		add_meta_box('reserved', 'Impostazione Risorsa Riservata', array('Spazi','spazio_crea_box_riservato'), 'spazi', 'advanced', 'high');
		add_meta_box('sigla', 'Sigla Spazio', array('Spazi','spazio_crea_box_sigla'), 'spazi', 'advanced', 'high');
	}	
...
// Recupero i dati dell'indisponibilità dello spazio
	...
	<br /><strong>Sigla</strong> <input type="text" id="sigla" name="sigla" value="' . esc_attr( $value[0] ) . '" size="5" />';	
	}

	function spazio_crea_box_riservato( $post ){
// Recupero i dati dell'indisponibilità dello spazio
	$riserv=get_post_meta($post->ID, "_riservato");
